...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    // Wait 500ms before starting so you can actually see it happen
  ...
  return (
    <div className="relative flex flex-col w-screen h-screen overflow-hidden bg-deep-bg text-text-primary">
      ...
      <nav className="relative z-10 flex items-center justify-between w-full px-12 py-6">
        ...
        {/* Right: Login Button (Updated to match the 20px Bold spec) */}
      ...
      <main className="relative z-10 flex flex-col items-center justify-center flex-1 px-4 text-center">
        ...
        {/* Search Input Bar. Hardcoded dimensions from Figma */}
        <div 
          className="flex items-center px-4 transition-all border bg-input/80 border-border backdrop-blur-sm focus-within:border-text-accent focus-within:shadow-[0_0_20px_rgba(169,189,232,0.15)]"
        ...
        >
          ...
          <input 
            ...
            className="flex-1 text-base bg-transparent outline-none text-text-primary placeholder:text-text-secondary"
          />
          <button 
            className="p-2 ml-2 transition-colors rounded-full bg-elevated text-text-secondary hover:text-text-primary"
