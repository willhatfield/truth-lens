...
def _score_single_cluster(
    ...
    total_unique_models = len(set(
        claim.model_id for claim in req.claims.values()
    ))
    independence = compute_independence_score_fn(len(supporting), total_unique_models)
    consistency = compute_consistency_score_fn(best_contra)
...
async def http_extract_claims(
    request: Request,
    token: HTTPAuthorizationCredentials = Depends(auth_scheme),
) -> dict:
    """HTTP POST endpoint for extract_claims."""
    validate_bearer_token(token)
    body = await request.json()
    return extract_claims.local(body)
...
@modal.fastapi_endpoint(method="POST")
async def http_embed_claims(
    request: Request,
    token: HTTPAuthorizationCredentials = Depends(auth_scheme),
) -> dict:
    """HTTP POST endpoint for embed_claims."""
    validate_bearer_token(token)
    body = await request.json()
    return embed_claims.local(body)
...
@modal.fastapi_endpoint(method="POST")
async def http_rerank_evidence_batch(
    request: Request,
    token: HTTPAuthorizationCredentials = Depends(auth_scheme),
) -> dict:
    """HTTP POST endpoint for rerank_evidence_batch."""
    validate_bearer_token(token)
    body = await request.json()
    return rerank_evidence_batch.local(body)
...
@modal.fastapi_endpoint(method="POST")
async def http_nli_verify_batch(
    request: Request,
    token: HTTPAuthorizationCredentials = Depends(auth_scheme),
) -> dict:
    """HTTP POST endpoint for nli_verify_batch."""
    validate_bearer_token(token)
    body = await request.json()
    return nli_verify_batch.local(body)
...
@modal.fastapi_endpoint(method="POST")
async def http_cluster_claims(
    request: Request,
    token: HTTPAuthorizationCredentials = Depends(auth_scheme),
) -> dict:
    """HTTP POST endpoint for cluster_claims."""
    validate_bearer_token(token)
    body = await request.json()
    return cluster_claims.local(body)
...
@modal.fastapi_endpoint(method="POST")
async def http_compute_umap(
    request: Request,
    token: HTTPAuthorizationCredentials = Depends(auth_scheme),
) -> dict:
    """HTTP POST endpoint for compute_umap."""
    validate_bearer_token(token)
    body = await request.json()
    return compute_umap.local(body)
...
@modal.fastapi_endpoint(method="POST")
async def http_score_clusters(
    request: Request,
    token: HTTPAuthorizationCredentials = Depends(auth_scheme),
) -> dict:
    """HTTP POST endpoint for score_clusters."""
    validate_bearer_token(token)
    body = await request.json()
    return score_clusters.local(body)
